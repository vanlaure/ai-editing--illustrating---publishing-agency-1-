...
export class OpenRouterProvider implements IAiProvider {
  ...
  async generateText(options: TextGenerationOptions): Promise<string> {
    const model = options.model || this.config.models?.textGeneration || 'openrouter/polaris-alpha';
  ...
  async generateImages(): Promise<string[]> {
    throw new Error('OpenRouter provider does not support image generation');
  }

  async generateAudio(): Promise<string> {
    throw new Error('OpenRouter provider does not support audio generation');
  }

  async editImage(): Promise<string> {
    throw new Error('OpenRouter provider does not support image editing');
  }

  createChatSession(options: ChatOptions): any {
    const model = options.model || this.config.models?.textGenerationFast || 'openrouter/polaris-alpha';
    return {
      model,
  ...
  async listModels(): Promise<ModelInfo[]> {
    ...
    const data = await response.json();
    return data.data
      .filter((model: any) => !model.pricing?.prompt || parseFloat(model.pricing.prompt) === 0)
      .map((model: any) => ({
        id: model.id,
