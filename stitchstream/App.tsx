...
import { VideoClip, AIAnalysis, ProcessingState, TitleDesign, ClosingCredits, CinematicEffects } from './types';
import { analyzeMontage } from './services/geminiService';
import { FILTER_LABELS, TRANSITION_LABELS, FONT_STYLES } from './constants';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (analysis) {
        ...
        // Initialize editable fields from AI
  ...
  // Accept clips pushed from parent app via postMessage
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      const incoming: VideoClip[] = event.data.clips
        .filter((c: any) => c && c.url && c.thumbnail)
        .map((c: any) => ({
          id: String(c.id),
          url: String(c.url),
          thumbnail: String(c.thumbnail),
          duration: Number(c.duration) || 1,
          name: String(c.name || `Clip ${c.id}`)
        }));

      if (incoming.length === 0) return;

      setClips(incoming);
      setActiveClipId(incoming[0].id);
      setAnalysis(null);
      setProcessingState({ isProcessing: false, progress: 0, status: 'idle' });
    };
  ...
  const handleApplyAIOrder = () => {
    ...
    // Map indices to current clip objects
    // Note: This logic assumes the analysis was run on the CURRENT set of clips in their original state.
    // If the user added/removed clips since analysis, this might be buggy, so we validate.
  ...
  const handleAIAnalyze = async () => {
    ...
    try {
      ...
      const result = await analyzeMontage(thumbnails);
      setAnalysis(result);
      ...
      <div className="max-w-[1600px] mx-auto p-6 md:p-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-800 pb-6">
          <div className="flex items-center gap-4">
             <div className="w-12 h-12 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-xl flex items-center justify-center shadow-lg shadow-purple-900/20">
             ...
             <div>
                <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">
                  StitchStream <span className="text-sm font-medium text-purple-400 ml-2 border border-purple-500/30 bg-purple-500/10 px-2 py-0.5 rounded-full">AI Studio</span>
                </h1>
                <p className="text-sm text-slate-500 mt-1">Intelligent Montage & Color Grading Agent</p>
             </div>
          </div>
          ...
          <div className="flex gap-3">
             <button
               ...
               className={`relative overflow-hidden flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-all group ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Sidebar: AI Dashboard & Upload */}
          <div className="lg:col-span-4 space-y-6">
             
             {/* AI Director's Notes */}
             {analysis && (
                 <div className="bg-gradient-to-b from-slate-800 to-slate-900 border border-slate-700/50 rounded-xl overflow-hidden shadow-2xl animate-fade-in">
                     <div className="p-4 border-b border-slate-700/50 flex justify-between items-center bg-black/20">
                         ...
                         <label className="flex items-center gap-2 cursor-pointer select-none">
                             <span className="text-xs text-slate-400">Apply AI Look</span>
                             <div 
                     ...
                     <div className="p-5 space-y-6 h-[600px] overflow-y-auto scrollbar-hide">
                         
                         {/* Suggested Order Alert */}
                         {JSON.stringify(analysis.directorNotes.suggestedClipOrder) !== JSON.stringify(clips.map((_, i) => i)) && (
             ...
             {!analysis && clips.length > 0 && (
                 <div className="bg-slate-800/30 border border-dashed border-slate-700 rounded-xl p-6 text-center text-slate-500 text-sm">
